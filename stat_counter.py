...
def save_table(table, file_name, folder_name="result"):
    with open(join(CURRENT_DIRECTORY, folder_name, file_name), "w") as f:
...
def save_spans(s1_copy, s2_copy, filename):
    with open(join(CURRENT_DIRECTORY, "span_results", filename), "a") as f:
...
def save_spans_counters(counter, counters_deleted, counters_file1_left,
    ...
    with open(join(CURRENT_DIRECTORY, "span_results", outputfile), "a") as f:
...
if __name__ == "__main__":
    ...
    for pair_of_files in results:
        ...
        save_table(tech_table, tech_output, "result_tech")
